...
def closer_city(lat, lon, city1, city2):
    ...
    """
    "*** YOUR CODE HERE ***"
    city = make_city("new_city", lat, lon)
...
def put_piece(board, max_rows, column, player):
    ...
    """
    "*** YOUR CODE HERE ***"
    modified_row = max_rows - 1
    while modified_row >= 0 and get_piece(board, modified_row, column) != '-':
        modified_row -= 1
    if modified_row != -1:
        new_row = replace_elem(board[modified_row], column, player)
        new_board = replace_elem(board, modified_row, new_row)
        return modified_row, new_board
    else:
        return -1, board 
...
def print_board(board, max_rows, max_cols):
    ...
    for row in range(max_rows):
        ...
        for column in range(max_cols):
            s = s + get_piece(board, row, column) + " "
        s = s.strip()
        print(s)
