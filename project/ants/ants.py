...
class ThrowerAnt(Ant):
    ...
    # def __init__(self, min_range=0, max_range=10000):
    #     Ant.__init__(self)
    ...
    def nearest_bee(self, hive):
        ...
        if place is hive or distance > self.max_range:
            return None
        else:
            return bee
        # END Problem 3B
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        ...
        "*** REPLACE THIS LINE ***"
        Ant.__init__(self)
        if QueenAnt.true_queue:
            QueenAnt.true_queue = False
            self.istrue = True
            self.ants = []
        else: 
            self.istrue = False
    ...
    def doubel_damage(self, ant):
        if not ant:
            return 
        if not ant in self.ants:
            ant.damage *= 2
            self.ants.append(ant)
        if ant.container:
            contain_ant = ant.ant
            if contain_ant and not contain_ant in self.ants:
                contain_ant.damage *= 2
                self.ants.append(contain_ant)
